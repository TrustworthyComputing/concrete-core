...
engine_error! {
    GlweCiphertextVectorDiscardingConversionGpuError for GlweCiphertextVectorDiscardingConversionGpuEngine @
...
impl<EngineError: std::error::Error> GlweCiphertextVectorDiscardingConversionGpuError<EngineError> {
...
///
/// # Formal Definition
pub trait GlweCiphertextVectorDiscardingConversionGpuEngine<Input, Output>: AbstractEngine
where
    Input: GlweCiphertextVectorEntity,
    Output: GlweCiphertextVectorEntity,
{
    ...
    fn discard_convert_glwe_ciphertext_vector(
        ...
        output: &mut Output,
        input: &Input,
    ) -> Result<(), GlweCiphertextVectorDiscardingConversionGpuError<Self::EngineError>>;

    /// Unsafely converts a GLWE ciphertext vector .
    ///
    /// # Safety
    /// For the _general_ safety concerns regarding this operation, refer to the different variants
    /// of [`GlweCiphertextVectorDiscardingConversionGpuError`]. For safety concerns _specific_ to
