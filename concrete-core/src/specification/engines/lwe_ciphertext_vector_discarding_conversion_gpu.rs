...
engine_error! {
    LweCiphertextVectorDiscardingConversionGpuError for LweCiphertextVectorDiscardingConversionGpuEngine @
...
impl<EngineError: std::error::Error> LweCiphertextVectorDiscardingConversionGpuError<EngineError> {
...
///
/// # Formal Definition
pub trait LweCiphertextVectorDiscardingConversionGpuEngine<Input, Output>: AbstractEngine
where
    Input: LweCiphertextVectorEntity,
    Output: LweCiphertextVectorEntity,
{
    ...
    fn discard_convert_lwe_ciphertext_vector(
        ...
        output: &mut Output,
        input: &Input,
    ) -> Result<(), LweCiphertextVectorDiscardingConversionGpuError<Self::EngineError>>;
