...
use crate::specification::engines::{
    LweCiphertextVectorDiscardingNandEngine, LweCiphertextVectorDiscardingNandError,
};

impl
    LweCiphertextVectorDiscardingNandEngine<
    ...
    > for CudaEngine
{
    fn discard_nand_lwe_ciphertext_vector(
        &mut self,
    ...
    ) -> Result<(), LweCiphertextVectorDiscardingNandError<CudaError>> {
        LweCiphertextVectorDiscardingNandError::perform_generic_checks(output, input_1, input_2)?;
        unsafe {
            self.discard_nand_lwe_ciphertext_vector_unchecked(output, input_1, input_2, bsk, ksk)
        };
        Ok(())
    }

    unsafe fn discard_nand_lwe_ciphertext_vector_unchecked(
        &mut self,
        output: &mut CudaLweCiphertextVector32,
        input_1: &CudaLweCiphertextVector32,
        input_2: &CudaLweCiphertextVector32,
        ...
        self.streams[self.get_curr_stream_idx()]
            .write()
            .unwrap()
            .discard_nand_amortized_lwe_ciphertext_vector::<u32>(
